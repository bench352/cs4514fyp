...
function PasswordChangeSuccessDialog(props: { open: boolean }) {
    ...
    return (
        <Dialog open={props.open} fullWidth maxWidth="xs">
            ...
            <DialogContent>
                <DialogContentText>
                    Your password has been changed successfully. Just to be safe, you will be logged out now. Please
...
export default function ChangePasswordDialog(props: ChangePasswordDialogProps) {
    ...
    const changePassword = async (data: ChangePasswordForm) => {
        try {
            ...
            await emaService.changePassword(token, data.oldPassword, data.newPassword);
        ...
        <Dialog open={props.open} fullWidth maxWidth="xs">
            ...
            <DialogContent>
                <Stack
                    ...
                    sx={{marginTop: 2}}
                >
                    <TextField id="current-password" label="Current Password" variant="standard" type="password"
                               autoComplete="current-password" {...register("oldPassword")}/>
                    <TextField
                        id="new-password"
                        variant="standard"
                        type="password"
            ...
            <DialogActions>
                ...
                >
                    ...
                    <Tooltip title="Apply">
                        <IconButton aria-label="save" size="large" onClick={handleSubmit(changePassword)}>
