...
function HealthinessPointDisplay(props: {
  telemetryKey: string;
  isAnomaly: boolean;
}) {
  return (
    <Box sx={{ padding: "10px" }}>
      ...
      >
        ...
        <Typography variant="h4" gutterBottom>
          {props.isAnomaly ? (
            <WarningAmberIcon sx={{ color: "#f57f17", fontSize: "40px" }} />
          ) : (
            <CheckCircleOutlineOutlinedIcon
              sx={{ color: "#4caf50", fontSize: "40px" }}
            />
          )}
        </Typography>
      </Stack>
...
export default function HealthinessCard(props: {
  healthiness: HealthinessCardEntry;
}) {
    ...
    <Card sx={{ marginBottom: "10px" }}>
      ...
      {props.healthiness.latestValues.size > 0 ? (
        <Stack
          ...
          spacing={0}
        >
          {Array.from(props.healthiness.latestValues.entries()).map(
            ([key, value]) => (
              <HealthinessPointDisplay
                key={key}
                telemetryKey={key}
                isAnomaly={value}
              />
