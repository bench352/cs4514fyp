...
function AnomalyKeyChartPaper(props: { anomalyResultPresentation: AnomalyResultPresentation }) {
    ...
    return (
        <Paper variant="outlined" sx={{padding: "10px"}}>
            ...
            <Chart
                type="scatter"
                data={{
                    labels: props.anomalyResultPresentation.values.map((v) => v.timestamp),
                    datasets: [{
                        ...
                        type: "scatter",
                        data: props.anomalyResultPresentation.values.map((v) => v.value),
                        pointRadius: props.anomalyResultPresentation.values.map((v) => v.isAnomaly ? 5 : 0),
...
export default function HealthinessDetailPage(props: BasePageProps) {
    ...
    let [resultPresentation, setResultPresentation] = useState([] as AnomalyResultPresentation[]);
    const [queryFromTs, setQueryFromTs] = useState(DateTime.now().minus({minute: 30}).toISO().slice(0, 16));
    const [queryToTs, setQueryToTs] = useState(DateTime.now().toISO().slice(0, 16));
    const queryData = async () => {
        try {
            ...
            let telemetryResult = await getHistoricalData(token, id, dtObjFromTs, dtObjToTs);
            let anomalyResult = await getHistoricalAnomalyDetectionResult(token, id, dtObjFromTs, dtObjToTs);
            setDevice(currentDevice);
            let resultPresentation: AnomalyResultPresentation[] = [];
            telemetryResult.data.forEach((telemetryKey) => {
                let anomalyKey = anomalyResult.data.find((anomaly) => anomaly.key === telemetryKey.key);
                let anomalyValues: AnomalyValues[] = [];
                telemetryKey.values.forEach((telemetryDataPoint) => {
                    let anomalyValue = anomalyKey?.values.find((anomalyDataPoint) => anomalyDataPoint.timestamp === telemetryDataPoint.timestamp);
                    if (anomalyValue) {
                        anomalyValues.push({
    ...
    return (
        <Container>
            <Button size="large" startIcon={<ArrowBackOutlinedIcon/>} color="secondary" onClick={() => {
            ...
            >
                <Card sx={{padding: "10px"}}>
                    ...
                    >
                        ...
                        <TextField variant="standard" type="datetime-local" value={queryFromTs} onChange={
                            (e) => {
                        ...
                        <TextField variant="standard" type="datetime-local" value={queryToTs} onChange={
                            (e) => {
                                setQueryToTs(e.target.value);
                            }
                        }/>
                        <Button variant="contained" startIcon={<SearchOutlinedIcon/>} onClick={queryData}>Query</Button>
                    </Stack>
                </Card>
                {
                    resultPresentation.map((result) => (
                        <AnomalyKeyChartPaper key={result.key} anomalyResultPresentation={result}/>
                    ))
                }
            </Stack>
        </Container>
