...
import HealthinessCard, {HealthinessCardEntry} from "../Components/Cards/HealthinessCard";
...
export default function HealthinessPage(props: BasePageProps) {
    ...
    const connectionStatus = {
        [ReadyState.CONNECTING]: <Chip icon={<SyncOutlinedIcon/>} size="small" variant="outlined" color="warning"
                                       label="Connecting"/>,
        [ReadyState.OPEN]: <Chip icon={<DoneOutlinedIcon/>} size="small" variant="outlined" color="success"
                                 label="Connected"/>,
        [ReadyState.CLOSING]: <Chip icon={<SyncProblemOutlinedIcon/>} size="small" variant="outlined" color="warning"
                                    label="Disconnecting"/>,
        [ReadyState.CLOSED]: <Chip icon={<SyncDisabledOutlinedIcon/>} size="small" variant="outlined" color="error"
                                   label="Disconnected"/>,
        [ReadyState.UNINSTANTIATED]: <Chip icon={<ErrorOutlineOutlinedIcon/>} size="small" variant="outlined"
                                           color="error"
                                           label="Uninstantiated"/>,
    };
    const [healthinessCardMap, setHealthinessCardMap] = useState(new Map<string, HealthinessCardEntry>());
    const {
        lastJsonMessage,
        readyState,
    } = useWebSocket(`${env.REACT_APP_DEVICE_HEALTH_SERVICE_WS_URL}/real-time?token=` + token);
    const updateHealthinessCardMap = (anomalyResult: Anomaly) => {
        ...
        anomalyResult.data.forEach(anomalyKey => {
            newMap.get(anomalyResult.deviceId)?.latestValues.set(anomalyKey.key, anomalyKey.values[0].isAnomaly);
        })
            ...
            >
                ...
                <div>
                    ...
                    <Typography variant="body1" gutterBottom>
                        Monitor the healthiness of your devices using real-time anomaly detection.
                    </Typography></div>
